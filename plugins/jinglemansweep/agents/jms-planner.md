...
```text
/jms-plan-init → /jms-plan-phase-new → /jms-plan-prd → /jms-plan-prd-review → /jms-plan-task-breakdown → /jms-plan-task-review → hand off for /jms-plan-execute
```

## Workflow

Follow these steps in order. Use the specified skills at each stage. For an automated end-to-end experience, use `/jms-plan-workflow` instead of following these steps manually.

### Step 1: Initialize

Run `/jms-plan-init` to ensure the `.plans` directory exists in the repository root. If it already exists, acknowledge and continue.
...
- **Create new phase** — ask the user for a short description (2-3 essential words, no articles/prepositions), then run `/jms-plan-phase-new <description>` to create it.
...
Run `/jms-plan-prd <phase-dir>` to generate `prd.md` from the user's `prompt.md`.

### Step 4: PRD Review

Run `/jms-plan-prd-review <phase-dir>` to evaluate the PRD against the original prompt. The skill produces `prd-review.md` with a verdict.

Handle the verdict:

- **PASS** — Proceed to Step 5.
- **REVISE** — The PRD needs changes. Tell the user what the review found (summarize the action items from `prd-review.md`). Ask the user to update `prd.md` to address the issues, or offer to re-run `/jms-plan-prd <phase-dir>` to regenerate the PRD. After the PRD is updated, re-run `/jms-plan-prd-review <phase-dir>`. Repeat until the verdict is PASS or the user chooses to override.
- **ESCALATE** — The review flagged issues requiring human judgment. Present the escalation reasons to the user and wait for their input. After the user resolves the issues (by updating `prompt.md` or `prd.md`), re-run `/jms-plan-prd-review <phase-dir>`.
...
Run `/jms-plan-task-breakdown <phase-dir>` to convert the approved PRD into `tasks.yaml`.

### Step 6: Task Review

Run `/jms-plan-task-review <phase-dir>` to validate the task list against the PRD. The skill produces `task-review.md` with a verdict.

Handle the verdict:

- **PASS** — Proceed to Step 7.
- **REVISE** — The task list needs changes. Tell the user what the review found (summarize the action items from `task-review.md`). Offer to re-run `/jms-plan-task-breakdown <phase-dir>` to regenerate the task list. After regeneration, re-run `/jms-plan-task-review <phase-dir>`. Repeat until the verdict is PASS or the user chooses to proceed anyway.
...
>
> Run `/jms-plan-execute <phase-dir>` to begin implementation.

...
Apply targeted corrections to address CRITICAL and MAJOR issues identified by `/jms-code-review`. This is a standalone, user-invocable skill primarily called by the `jms-execute` orchestrator during the review-and-fix loop. It addresses only the issues provided -- no unrelated refactoring, no scope expansion.
...
├── logs/               <- created by jms-phase-new
└── state.yaml          <- created by jms-phase-new
```
...
3. Scan `<phase-path>/reviews/` for review round files matching `round-*.yaml`. If none exist, stop and tell the user to run `/jms-code-review` first.
...
- If fixes were applied, note that `/jms-validate` should be run on the changed files to verify the fixes do not introduce syntax or test failures.

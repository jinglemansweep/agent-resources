...
├── tasks.yaml          <- input (task list from jms-plan-task-breakdown)
...
│   ├── round-2.yaml
│   └── ...
└── summary.md          <- output (final report from jms-plan-summary)
```
...
4. If no qualifying phase directories are found, stop and tell the user to run `/jms-plan-task-breakdown` first to generate a task list.

### Step 2: Validate Inputs

Verify the phase directory contains the required files:

1. `tasks.yaml` -- If missing, stop and tell the user to run `/jms-plan-task-breakdown` first.
2. `prd.md` -- If missing, stop and tell the user to run `/jms-plan-prd` first.
3. `state.yaml` -- If missing, stop and tell the user to run `/jms-plan-phase-new` first.
...
All tasks are delegated to the `jms-developer` agent. The Developer agent handles domain-specific routing internally via its signal-based skill loading mechanism.
...
1. The agent identity: "You are acting as the jms-developer agent. Follow the instructions in your agent definition."
...
```text
You are acting as the jms-developer agent. Follow the instructions in your agent definition.
...
**Round 1:** Run `/jms-plan-code-review` with scope `full`:

```text
/jms-plan-code-review <phase-path> full
```

**Round 2+:** Run `/jms-plan-code-review` with scope `changed`:

```text
/jms-plan-code-review <phase-path> changed
```
...
If CRITICAL or MAJOR issues exist, run `/jms-plan-fix`:
...
Run `/jms-plan-summary` to generate the final report:
...
- **MINOR issues never trigger fixes.** MINOR issues from code review are logged in the summary but are never passed to `/jms-plan-fix` and never cause additional review rounds.
- **Fix only what review finds.** The review-and-fix loop addresses issues found by `/jms-plan-code-review`. Do not add extra fixes, refactoring, or improvements beyond what is reported.

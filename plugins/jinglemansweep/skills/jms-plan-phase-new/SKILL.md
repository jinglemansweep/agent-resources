...
```text
/jms-plan-phase-new <description>
```
...
Ensure the `<repo-root>/.plans` directory exists. If not, stop and tell the user to run `/jms-plan-init` first.
...
- A reminder to write their requirements in `prompt.md` before running `/jms-plan-prd`

...
- `<file-paths>` -- One or more file paths (space-separated) that were created or modified by the current task. These are provided by the caller (typically `jms-plan-execute`).
...
- **Report only, never fix.** This skill detects and reports problems. It must not modify any files, auto-fix lint errors, or attempt corrections. Fixes are the responsibility of the implementing agent or `/jms-plan-fix`.

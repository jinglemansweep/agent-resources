...
```text
/jms-plan-workflow [phase-path-or-description]
```
...
| Stage | Skill | Purpose |
|-------|-------|---------|
| 1 | `/jms-plan-init` | Ensure `.plans/` directory exists |
| 2 | `/jms-plan-phase-new` | Create a new phase directory (skipped if using existing phase) |
| 3 | `/jms-plan-prd` | Generate PRD from `prompt.md` |
| 4 | `/jms-plan-prd-review` | Review the PRD |
| 5 | `/jms-plan-task-breakdown` | Convert PRD to task list |
| 6 | `/jms-plan-task-review` | Validate the task list |
| 7 | `/jms-plan-execute` | Execute all tasks |
| 8 | `/jms-plan-summary` | Generate final summary report |
...
**Stage 1 -- Init:** Run `/jms-plan-init` to ensure the `.plans/` directory exists. Report: "Stage 1/8: Initializing .plans directory..."

**Stage 2 -- New phase:** Run `/jms-plan-phase-new <description>` with the user's description. Record the returned phase path. Report: "Stage 2/8: Created phase at `<phase-path>`"
...
If `state.yaml` has `status: running` or `status: review_loop`, Stage 7 (`/jms-plan-execute`) will handle its own resumption internally.

### Step 4: Generate PRD (Stage 3)

Report: "Stage 3/8: Generating PRD..."

Run `/jms-plan-prd <phase-path>`.

### Step 5: Review PRD (Stage 4)

Report: "Stage 4/8: Reviewing PRD..."

Run `/jms-plan-prd-review <phase-path>`.

Read the output `<phase-path>/prd-review.md` and check the verdict:

- **PASS**: Continue to Step 6.
- **REVISE**: Report the action items to the user. Re-run `/jms-plan-prd <phase-path>` to regenerate the PRD incorporating the feedback, then re-run `/jms-plan-prd-review <phase-path>`. Repeat up to 2 times. If still REVISE after 2 retries, escalate to the user.
...
Run `/jms-plan-task-breakdown <phase-path>`.

### Step 7: Review Task List (Stage 6)

Report: "Stage 6/8: Reviewing task list..."

Run `/jms-plan-task-review <phase-path>`.

Read the output `<phase-path>/task-review.md` and check the verdict:

- **PASS**: Continue to Step 8.
- **REVISE**: Report the action items to the user. Re-run `/jms-plan-task-breakdown <phase-path>` to regenerate the task list incorporating the feedback, then re-run `/jms-plan-task-review <phase-path>`. Repeat up to 2 times. If still REVISE after 2 retries, escalate to the user.

### Step 8: Execute Tasks (Stage 7)

Report: "Stage 7/8: Executing tasks..."

Run `/jms-plan-execute <phase-path>`.

This stage handles its own resumption internally via `state.yaml`. It also runs the review-and-fix loop and produces the summary as part of its pipeline. If `/jms-plan-execute` completes successfully (including its internal summary generation), skip Step 9.

### Step 9: Generate Summary (Stage 8)

Report: "Stage 8/8: Generating summary..."

Run `/jms-plan-summary <phase-path>` only if `/jms-plan-execute` did not already generate a summary.
...
- **Delegate, do not implement.** This skill orchestrates by invoking other `/jms-plan-*` skills. It does not generate PRDs, break down tasks, or execute implementations itself.

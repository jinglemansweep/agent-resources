...
Ensure the `<repo-root>/.plans` directory exists. If not, stop and tell the user to run `/jp-plan-init` first.
...
- A reminder to write their requirements in `prompt.md` before running `/jp-plan-prd`

...
```text
/jp-plan-workflow [phase-path-or-description]
```
...
| Stage | Skill | Purpose |
|-------|-------|---------|
| 1 | `/jp-plan-init` | Ensure `.plans/` directory exists |
| 2 | `/jp-plan-new` | Create a new phase directory (skipped if using existing phase) |
| 3 | `/jp-plan-prd` | Generate PRD from `prompt.md` |
| 4 | `/jp-plan-prd-review` | Review the PRD |
| 5 | `/jp-plan-task-breakdown` | Convert PRD to task list |
| 6 | `/jp-plan-task-review` | Validate the task list |
| 7 | `/jp-plan-execute` | Execute all tasks |
| 8 | `/jp-plan-summary` | Generate final summary report |
...
**Stage 1 -- Init:** Run `/jp-plan-init` to ensure the `.plans/` directory exists. Report: "Stage 1/8: Initializing .plans directory..."

**Stage 2 -- New phase:** Run `/jp-plan-new <description>` with the user's description. Record the returned phase path. Report: "Stage 2/8: Created phase at `<phase-path>`"
...
If `state.yaml` has `status: running` or `status: review_loop`, Stage 7 (`/jp-plan-execute`) will handle its own resumption internally.

### Step 4: Generate PRD (Stage 3)

Report: "Stage 3/8: Generating PRD..."

Run `/jp-plan-prd <phase-path>`.

### Step 5: Review PRD (Stage 4)

Report: "Stage 4/8: Reviewing PRD..."

Run `/jp-plan-prd-review <phase-path>`.

Read the output `<phase-path>/prd-review.md` and check the verdict:

- **PASS**: Continue to Step 6.
- **REVISE**: Report the action items to the user. Re-run `/jp-plan-prd <phase-path>` to regenerate the PRD incorporating the feedback, then re-run `/jp-plan-prd-review <phase-path>`. Repeat up to 2 times. If still REVISE after 2 retries, escalate to the user.
...
Run `/jp-plan-task-breakdown <phase-path>`.

### Step 7: Review Task List (Stage 6)

Report: "Stage 6/8: Reviewing task list..."

Run `/jp-plan-task-review <phase-path>`.

Read the output `<phase-path>/task-review.md` and check the verdict:

- **PASS**: Continue to Step 8.
- **REVISE**: Report the action items to the user. Re-run `/jp-plan-task-breakdown <phase-path>` to regenerate the task list incorporating the feedback, then re-run `/jp-plan-task-review <phase-path>`. Repeat up to 2 times. If still REVISE after 2 retries, escalate to the user.

### Step 8: Execute Tasks (Stage 7)

Report: "Stage 7/8: Executing tasks..."

Run `/jp-plan-execute <phase-path>`.

This stage handles its own resumption internally via `state.yaml`. It also runs the review-and-fix loop and produces the summary as part of its pipeline. If `/jp-plan-execute` completes successfully (including its internal summary generation), skip Step 9.

### Step 9: Generate Summary (Stage 8)

Report: "Stage 8/8: Generating summary..."

Run `/jp-plan-summary <phase-path>` only if `/jp-plan-execute` did not already generate a summary.
...
- **Delegate, do not implement.** This skill orchestrates by invoking other `/jp-plan-*` skills. It does not generate PRDs, break down tasks, or execute implementations itself.

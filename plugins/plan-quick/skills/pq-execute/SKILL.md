...
Read `<plan-dir>/tasks.md`. If missing, stop and tell the user to run `pq-taskify` first.
...
> Context should be cleared before the next group. Run `/clear` then re-invoke `/pq-execute <plan-dir>` to continue.
